...
<html lang="en">
  ...
  ga('send', 'pageview');
	...
	<link href="all_pictures.css" rel="stylesheet">
		...
		<div class="panel-heading">
			<table width="100%"><tr>
				<td width="60%">
					...
					<font style="color:#000; font-size:17px;">All Pictures</font>
				</td>
...
<?php
	...
	mysql_select_db($dbname);
		...
		$query = "select image_name from picture order by id desc";
		$query2 = "select id from picture order by id desc";
		$query3 = "select date from picture order by id desc";
		$query4 = "select time from picture order by id desc";
	...
	$row4 = array();
		...
		while ($rowwww = mysql_fetch_array($result4))
		{
			...
			for($x = 0; $x < sizeof($row3); $x++)
			{
					...
					<div class=\"col-lg-4 col-md-6 col-sm-6 col-xs-12\" style=\" background-color:white; border-radius:1px; max-height:470px;\">
						
						<div style=\"width:90%; margin-top:8px;\" >
						<a href=\"http://www.fab8thproductions.com/pictures/".$row1[$x][0]."\" target=\"_blank\">
						<img src=\"pictures/". $row1[$x][0]."\" width=\"100%\" height=\"170\"/></a>
					
					
						<h6 style=\"font-size:13px; color:#ED188A; min-height:38px; margin:5px;\">". $row1[$x][0]."</h6>".
						
							"<hr class=\"featurette-divider\" style=\"background-color:#ED188A; margin:0px; height:2px;\">
								<p style=\"float:left; margin-top:3px; margin-bottom:3px; font-size:12px;\" >Posted : ".$row3[$x][0]."</p>
								<p style=\"float:right; margin-top:3px; margin-bottom:3px; font-size:12px;\">Time : ".$row4[$x][0]."</p>					
							<hr class=\"featurette-divider\" style=\"background-color:#ED188A; margin:0px; height:2px; clear:both;\">
