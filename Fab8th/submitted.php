<?php
	...
	$email = $_POST['email'];
	$phone = $_POST['phone'];
	$male = $_POST['no_of_male_ushers'];
	$female = $_POST['no_of_female_ushers'];
	$event = $_POST['event'];
	$colour1 = $_POST['colour_comb_1'];
	$colour2 = $_POST['colour_comb_2'];
	$c1 = $_POST['other_col1'];
	$c2 = $_POST['other_col2'];
	if(empty($title) || empty($email) || empty($male) || empty($female))
	...
	{
		if($colour1 == "other1" && $colour2 == "other2")
	{
		$query = "insert into ushering(title, surname, firstname, email, phone, male, female, event, colour1, colour2) 
		values('$title', '$surname', '$firstname', '$email', '$phone', '$male', '$female', '$event', '$c1', '$c2')";
	}
	else if($colour1 == "other1" && $colour2 != "other2")
	{
		$query = "insert into ushering(title, surname, firstname, email, phone, male, female, event, colour1, colour2) 
		values('$title', '$surname', '$firstname', '$email', '$phone', '$male', '$female', '$event', '$c1', '$colour2')";
	}
	else if($colour1 != "other1" && $colour2 == "other2")
	{
		$query = "insert into ushering(title, surname, firstname, email, phone, male, female, event, colour1, colour2) 
		values('$title', '$surname', '$firstname', '$email', '$phone', '$male', '$female', '$event', '$colour1', '$c2')";
	}
	else
	{
		$query = "insert into ushering(title, surname, firstname, email, phone, male, female, event, colour1, colour2) 
		values('$title', '$surname', '$firstname', '$email', '$phone', '$male', '$female', '$event', '$colour1', '$colour2')";
	}
	}
	...
	$txt = "Dear ".$title." ".$firstname." ".$surname . " ,\r\nThis is to inform you that your booking for Ushers has been received by Fab8th Productions team and you will be contacted shortly.\r\nThanks for your patronage.";
	...
	<body>
		<p>There is a new booking on www.fab8thproductions.com for Ushering services as detailed below</p>
			<table border=\"3\" bordercolor=\"#ED188A\">
				...
				<tr><td>No. of Male Ushers</td><td>$male</td></tr>
				<tr><td>No. of Female Ushers</td><td>$female</td></tr>
				<tr><td>Event/Occassion</td><td>$event</td></tr>
				<tr><td>Colour 1</td><td>$colour1 $c1</td></tr>
				<tr><td>Colour 2</td><td>$colour2 $c2</td></tr>
			</table>
	</body>
	</html>";
...
<html>
	...
	<link href="film_services.css" rel="stylesheet">
